# 前端页面样式与布局重构 Implementation Plan
...
**Tech Stack:** React 19, TypeScript, antd v6 (`@ant-design/icons` v6), Tailwind CSS v4, react-router-dom v6, @tanstack/react-query, ECharts（现有看板）, pnpm + turbo。
...
- Test: `apps/web/src/App.test.tsx`（reduced-motion 断言不得改动）

**Interfaces:**
- Produces: `aiHubTheme: ThemeConfig`（保持 `packages/ui/src/index.ts` 的 `export { aiHubTheme } from "./theme.js"` 不变，`providers.tsx` 消费方式不变）。
- Consumes: 无（只改 token 与 CSS）。
...
- Modify: `apps/web/src/router/routes.ts`（新增 `ROUTES.assistant` 与面包屑元表）
- Create: `apps/web/src/components/layout/Breadcrumbs.tsx`
- Create: `apps/web/src/components/layout/useBreadcrumbs.ts`
- Modify: `apps/web/src/router/index.tsx`（登录页移出 AppShell，置于根路由）
- Test: `apps/web/src/App.test.tsx`（更新面包屑/结构断言；banner 高度断言到 Task 3 再改）
...
- Consumes: 现有 `Header`、`Navigation` 组件原样保留（本任务不重写它们）。
...
**Files:**
- Modify: `apps/web/src/components/layout/Header.tsx`（重写）
- Create: `apps/web/src/modules/marketplace/search.store.ts`
- Create: `apps/web/src/modules/application/last-viewed.ts`
- Modify: `apps/web/src/App.test.tsx`（banner 高度断言、搜索框断言）
- Test: `apps/web/src/phase4.test.tsx`（搜索框 aria 名称保持"搜索应用"）
...
- Consumes: `useAuth()`（`actor.employeeId`、`logout`、`isLoading`）、`useNotifications()`（未读数 = `readAt === null` 的条数）、`readLastViewedApplicationId`。
...
**Files:**
- Modify: `apps/web/src/components/layout/Navigation.tsx`（重写为侧边栏菜单）
- Create: `apps/web/src/modules/auth/roles.ts`
- Modify: `apps/web/src/components/layout/AppShell.tsx`（折叠状态、折叠按钮、移动端 Drawer、公告区）
- Modify: `apps/web/src/test/icons.tsx`（新增本任务用到的图标）
- Test: `apps/web/src/App.test.tsx`、`identity-admin.test.tsx`（导航名称断言不变：应用市场/创新广场/站内通知/Organization→本任务暂不改文案，Task 7/9 再改）
...
- Consumes: `useAuth()`（`actor`）、`readLastViewedApplicationId()`（审核工作台入口）。
...
- Modify: `apps/web/src/pages/marketplace/MarketplaceHero.tsx`（改为排序标签条）
- Modify: `apps/web/src/pages/marketplace/MarketplaceFilters.tsx`
- Modify: `apps/web/src/pages/marketplace/AppCard.tsx`
- Modify: `apps/web/src/pages/marketplace/MarketplaceSidebar.tsx`
- Modify: `apps/web/src/test/icons.tsx`（如需新图标）
- Test: `apps/web/src/phase4.test.tsx`

**Interfaces:**
- Produces: `MarketplaceSortMode = "recommended" | "latest" | "popular"`（删除 `"rating"`）；`MarketplaceFilters` 的 `tagIds` 改为 `string[]` 多选。
- Consumes: `readSearchQuery()`（初始 query）、`useCatalogSearch`（`refetch` 用于重试）、`useDepartments()`。
...
- Create: `apps/web/src/components/common/ErrorBlock.tsx`
- Create: `apps/web/src/components/common/EmptyBlock.tsx`
- Create: `apps/web/src/components/common/ForbiddenBlock.tsx`
- Create: `apps/web/src/components/common/NotFoundBlock.tsx`
- Create: `apps/web/src/components/common/SkeletonCard.tsx`
- Create: `apps/web/src/components/common/SkeletonDetail.tsx`
- Create: `apps/web/src/components/common/KpiCard.tsx`
- Create: `apps/web/src/components/common/StatCard.tsx`
- Create: `apps/web/src/components/common/ConfirmModal.tsx`
- Create: `apps/web/src/components/common/index.ts`（统一导出）
...
- Test: `apps/web/src/App.test.tsx`（应用路由/生命周期断言）

**Interfaces:**
- Consumes: `PageHeader`（Task 6）、`ErrorBlock`、`EmptyBlock`。
- Produces: `ApplicationAdminPage` 新 props 不变（`{ title, description, children }`），内部改为 `PageHeader` + 中文页签 + 只读提示；页签 label：应用详情/版本管理/审核工作台/交付配置。
...
- Modify: `apps/web/src/pages/analytics/AnalyticsDashboardPage.tsx`
- Test: `apps/web/src/App.test.tsx`（创新页断言）、`apps/web/src/phase6.test.tsx`

**Interfaces:**
- Consumes: `PageHeader`、`KpiCard`、`StatCard`、`ErrorBlock`、`EmptyBlock`。
...
- Modify: `apps/web/src/modules/application/last-viewed.ts`（补写入函数）
- Test: `apps/web/src/identity-admin.test.tsx`、`apps/web/src/auth.test.tsx`

**Interfaces:**
- Produces: `last-viewed.ts` 追加 `rememberLastViewedApplicationId(id: string): void`（sessionStorage 写入，try/catch 容错）。
- Consumes: `PageHeader`、`ErrorBlock`、`EmptyBlock`、`ConfirmModal`、`rememberLastViewedApplicationId`。
...
**Files:**
- Create: `apps/web/src/pages/assistant/AssistantPage.tsx`
- Modify: `apps/web/src/router/index.tsx`（懒加载注册 `/assistant`）
- Modify: `apps/web/src/test/icons.tsx`（如缺图标）
- Test: 新增 `apps/web/src/assistant.test.tsx` 或在 `App.test.tsx` 加断言

**Interfaces:**
- Consumes: `ROUTES.assistant`、`PageHeader`。
- Produces: 默认导出 `AssistantPage`。
...
**Files:**
- Modify: `apps/web/src/test/icons.tsx`（补全所有新图标白名单）
- Modify: 相关测试文件（保证中文文案断言与实现一致）
- Modify: `processing_visualization.html`（根目录，见下方必填内容）
