...
describe("buildNotificationFixture", () => {
  ...
  it("produces 15 notifications", () => {
    expect(fixture.notifications).toHaveLength(15);
  });

  it("has exactly 3 notifications per employee (5 employees)", () => {
    const byEmployee = new Map<string, number>();
    for (const n of fixture.notifications) {
      const count = byEmployee.get(n.recipient_employee_id) ?? 0;
      byEmployee.set(n.recipient_employee_id, count + 1);
    }
    for (const [, count] of byEmployee) {
      expect(count).toBe(3);
    }
  ...
  it("has a mix of read and unread notifications", () => {
    ...
    expect(read.length + unread.length).toBe(15);
  });
  ...
  it("all recipient_employee_ids reference the 5 demo employees", () => {
    ...
    for (const n of fixture.notifications) {
      expect(validEmployees.has(n.recipient_employee_id)).toBe(true);
    }
  });
