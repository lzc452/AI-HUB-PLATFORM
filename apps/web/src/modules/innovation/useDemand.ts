...
function invalidateDemand(queryClient: ReturnType<typeof useQueryClient>, demandId?: string) {
  return Promise.all([
    ...
    demandId
      ...
      : Promise.resolve(),
    demandId
      ? queryClient.invalidateQueries({ queryKey: demandKeys.comments(demandId) })
      : Promise.resolve(),
  ]);
}
...
export function useLikeDemand(demandId: string | undefined) {
  ...
  return useMutation({
    ...
    onMutate: async () => {
      ...
      await queryClient.cancelQueries({ queryKey: demandKeys.detail(demandId) });
      const previous = queryClient.getQueryData(demandKeys.detail(demandId));
      queryClient.setQueryData(demandKeys.detail(demandId), (current: OptimisticDemand | undefined) => {
...
export function useAddDemandComment(demandId: string | undefined) {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (input: string | { body: string; parentCommentId?: string | null }) =>
      addDemandComment(
        demandId as string,
...
export function useLikeDemandComment(demandId: string | undefined) {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (commentId: string) => likeDemandComment(demandId as string, commentId),
    onMutate: async (commentId) => {
      if (!demandId) return undefined;
      await queryClient.cancelQueries({ queryKey: demandKeys.comments(demandId) });
      const previous = queryClient.getQueryData(demandKeys.comments(demandId));
      queryClient.setQueryData(demandKeys.comments(demandId), (comments: OptimisticComment[] | undefined) =>
        comments?.map((comment) =>
          comment.commentId === commentId
            ? {
                ...comment,
                likedByCurrentActor: !comment.likedByCurrentActor,
                likeCount: Math.max(0, comment.likeCount + (comment.likedByCurrentActor ? -1 : 1)),
              }
            : comment,
        ),
      );
    ...
    onError: (error, _commentId, context) => {
      if (demandId && context?.previous) {
        queryClient.setQueryData(demandKeys.comments(demandId), context.previous);
      }
...
export function useReviewDemand(demandId?: string) {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (input: Parameters<typeof reviewDemand>[1]) => reviewDemand(demandId as string, input),
    onSuccess: async () => invalidateDemand(queryClient, demandId),
...
export function useClaimDemand(demandId?: string) {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (expectedVersion?: number) => claimDemand(demandId as string, expectedVersion),
    onSuccess: async () => invalidateDemand(queryClient, demandId),
...
export function useAdvanceDemandStatus(demandId?: string) {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (input: { status: DemandStatus; expectedVersion: number; reason?: string }) =>
      advanceDemandStatus(demandId as string, input.status, input.expectedVersion, input.reason),
    onSuccess: async () => invalidateDemand(queryClient, demandId),
...
export function useSetDemandPriority(demandId?: string) {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (input: Parameters<typeof setDemandPriority>[1]) => setDemandPriority(demandId as string, input),
    onSuccess: async () => invalidateDemand(queryClient, demandId),
...
export function useReportDemand(demandId?: string) {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (input: Parameters<typeof reportDemand>[1]) => reportDemand(demandId as string, input),
    onSuccess: async () => {
      await invalidateDemand(queryClient, demandId);
...
export function useDemandGovernanceData(demandId?: string, enabled = false) {
  ...
  return {
    applications: useQuery({ ...options, queryKey: ["demands", "applications", demandId], queryFn: () => listDemandApplications(demandId as string) }),
    collaborators: useQuery({ ...options, queryKey: ["demands", "collaborators", demandId], queryFn: () => listDemandCollaborators(demandId as string) }),
    progress: useQuery({ ...options, queryKey: ["demands", "progress", demandId], queryFn: () => listDemandProgress(demandId as string) }),
    pilots: useQuery({ ...options, queryKey: ["demands", "pilots", demandId], queryFn: () => listDemandPilots(demandId as string) }),
    reports: useQuery({ ...options, queryKey: ["demands", "reports", demandId], queryFn: () => listDemandReports(demandId as string) }),
  };
}

function useGovernanceMutation<T>(demandId: string | undefined, mutationFn: (input: T) => Promise<unknown>, errorMessage: string) {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn,
    onSuccess: async () => invalidateDemand(queryClient, demandId),
...
export function useAddDemandCollaborator(demandId?: string) {
  return useGovernanceMutation(demandId, (input: Parameters<typeof addDemandCollaborator>[1]) => addDemandCollaborator(demandId as string, input), "添加协作者失败");
}

export function useUpdateDemandCollaboratorRole(demandId?: string) {
  return useGovernanceMutation(
    demandId,
    (input: Parameters<typeof updateDemandCollaboratorRole>[2] & { employeeId: string }) =>
...
export function useAddDemandProgress(demandId?: string) {
  return useGovernanceMutation(demandId, (input: Parameters<typeof addDemandProgress>[1]) => addDemandProgress(demandId as string, input), "新增进展失败");
}

export function useCreateDemandPilot(demandId?: string) {
  return useGovernanceMutation(demandId, (input: Parameters<typeof createDemandPilot>[1]) => createDemandPilot(demandId as string, input), "创建试点失败");
}

export function useUpdateDemandPilot(demandId?: string) {
  return useGovernanceMutation(demandId, (input: { pilotId: string; status?: string; outcome?: string; endsAt?: string | null }) => updateDemandPilot(demandId as string, input.pilotId, input), "更新试点失败");
}

export function useLinkDemandApplication(demandId?: string) {
  return useGovernanceMutation(demandId, (input: Parameters<typeof linkDemandApplication>[1]) => linkDemandApplication(demandId as string, input), "关联应用失败");
}

export function useCreateApplicationFromDemand(demandId?: string) {
  return useGovernanceMutation(demandId, (input: Parameters<typeof createApplicationFromDemand>[1]) => createApplicationFromDemand(demandId as string, input), "从需求创建应用失败");
}

export function useMergeDemand(demandId?: string) {
  return useGovernanceMutation(demandId, (input: Parameters<typeof mergeDemand>[1]) => mergeDemand(demandId as string, input), "合并需求失败");
}

export function useResolveDemandReport(demandId?: string) {
  return useGovernanceMutation(demandId, (input: { reportId: string; status: "dismissed" | "hidden" | "restored" }) => resolveDemandReport(demandId as string, input.reportId, input.status), "处理举报失败");
}

export function useLookupAnonymousAuthor(demandId?: string) {
  return useMutation({
    mutationFn: (input: { commentId: string }) => lookupAnonymousAuthor(demandId as string, input.commentId),
    onSuccess: ({ employeeId }) => showSuccessMessage(`匿名评论作者：${employeeId}`),
...
export function useRemoveDemandCollaborator(demandId?: string) {
  ...
  return useMutation({
    mutationFn: (input: { employeeId: string; expectedVersion?: number }) =>
      removeDemandCollaborator(demandId as string, input.employeeId, input.expectedVersion),
    onSuccess: async () => invalidateDemand(queryClient, demandId),
...
export function useRemoveDemandApplication(demandId?: string) {
  ...
  return useMutation({
    mutationFn: (input: { applicationId: string; expectedVersion?: number }) =>
      removeDemandApplication(demandId as string, input.applicationId, input.expectedVersion),
    onSuccess: async () => invalidateDemand(queryClient, demandId),
