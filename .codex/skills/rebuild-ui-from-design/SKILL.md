...
- 还没有查看原图原始分辨率就开始写 JSX/CSS。
- 还没有建立 CodeGraph 影响清单就修改 `theme.ts` 或全局 CSS。
- 以页面复制代替 `components/common` 公共组件。
- 用近似图标、emoji、字符或外部 icon font 代替设计图图标。
- 没有同视口截图和偏差矩阵就声明完成。
- 行为或公共接口已经改变，却把测试安排到实现之后。

这些信号表示当前页面尚未满足完成契约；保留业务逻辑不等于跳过视觉证据。

## 常见错误与修正

| 错误 | 修正 |
| --- | --- |
| 只读缩略图，凭印象调间距 | 用原始 PNG 尺寸建立 `DesignMeasurement`，逐区记录坐标和尺寸 |
| 只修当前页面的 Ant Design 外观 | 先列出调用点和受影响路由，再决定 token、component token 或作用域 CSS |
| 公共组件内部请求页面数据 | 页面或 `modules` 获取数据，通过 props 注入公共组件 |
| SVG 能显示但轮廓比例不一致 | 记录 `viewBox`、path、stroke、fill、linecap、linejoin 和 1px 对齐结果 |
| 只验收静态截图 | 追加加载、空、错误、无权限、hover、focus、selected、disabled 状态验证 |
